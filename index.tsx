...
const App = () => {
  ...
  const handleAiAnalyze = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        전문 MD의 입장에서 현재 마진 구조의 건강함과 시장 경쟁력을 한국어로 3문장 이내 요약 조언하세요.
      `;
      ...
      setAiAnalysis(response.text || "분석 실패");
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fcfdfe]">
      <header className="bg-slate-900 text-white px-8 py-6 sticky top-0 z-50 shadow-xl flex justify-between items-center border-b border-white/5 backdrop-blur-md bg-opacity-95">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-1 flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Intelligent Pricing v9.0
            </p>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-6 lg:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <aside className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 overflow-hidden relative">
            <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-50 rounded-full -mr-16 -mt-16 opacity-40 blur-2xl"></div>
            <h2 className="font-black text-slate-800 text-[10px] mb-8 flex items-center gap-2 uppercase tracking-widest">
              <Settings2 className="w-3.5 h-3.5 text-indigo-600" /> 가격 전략 시뮬레이션
            </h2>
            
            <div className="space-y-6 relative z-10">
              <div className="p-6 bg-indigo-600 rounded-3xl shadow-xl shadow-indigo-500/20 group transition-all hover:scale-[1.02]">
                <label className="text-[9px] font-black text-indigo-100 uppercase mb-2 block tracking-widest">목표 소비자 가격 (역산)</label>
                <div className="relative">
                  <span className="absolute left-0 top-1 text-white/40 font-bold text-xl font-mono">₩</span>
                  <input 
                    type="number" 
                    value={targetPrice} 
                    onChange={handleTargetPriceChange} 
                    className="w-full pl-8 bg-transparent border-b border-indigo-400/50 text-white text-3xl font-black outline-none font-mono focus:border-white transition-all pb-1"
                  />
                </div>
              </div>
        ...
        <section className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
              ...
              <span className="text-slate-400 text-[9px] font-black uppercase tracking-widest block mb-1">권장 소비자가</span>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
              ...
              <span className="text-slate-400 text-[9px] font-black uppercase tracking-widest block mb-1">예상 마진액</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm h-[400px] flex flex-col relative">
              <div className="flex justify-between items-center mb-6">
                ...
                <button 
                  ...
                  className="text-[9px] font-black bg-slate-900 text-white px-4 py-2 rounded-xl hover:bg-indigo-600 disabled:opacity-50 transition-all flex items-center gap-2"
                >
                  {isAnalyzing ? <RefreshCcw className="w-3 h-3 animate-spin"/> : <Sparkles className="w-3 h-3"/>}
                  AI 조언
                </button>
              </div>
              <div className="flex-1 bg-slate-50/70 rounded-2xl p-6 overflow-y-auto custom-scroll border border-slate-100/50">
                {error ? (
                  <p className="text-xs font-bold text-red-500">{error}</p>
                ) : isAnalyzing ? (
                  <div className="flex items-center justify-center h-full text-indigo-400 font-black text-[10px] animate-pulse uppercase tracking-widest">분석 중...</div>
                ) : (
                  <div className="text-[13px] text-slate-600 leading-relaxed font-medium">
                    {aiAnalysis || "가격을 조정한 후 AI 조언 버튼을 클릭하세요."}
                  </div>
                )}
              </div>
            </div>
      ...
      <footer className="p-10 text-center border-t border-slate-100 bg-white/50 mt-auto">
        <p className="text-[9px] font-black text-slate-300 tracking-[0.5em] uppercase">
          © 2025 Smart-Price Pro • Professional Pricing Logic
        </p>
      </footer>
    </div>
  );
